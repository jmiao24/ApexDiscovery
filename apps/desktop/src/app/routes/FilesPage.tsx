...
/**
 * Workspace file explorer (P2-1: multi-file project ergonomics). Browse the whole
 * workspace as a tree and open ANY file in the native previewers — not just the
 * files the agent happened to mention. Directories are navigable via a breadcrumb;
 * files open in the same viewers used elsewhere (figures, tables, PDF, molecule,
 * genome tracks, notebooks), so a larger project is workable in one place.
 */
export function FilesPage() {
  const [dir, setDir] = useState(""); // workspace-relative; "" = root
  ...
  return (
    <div className="flex h-full min-h-0">
      ...
      <div className="min-h-0 flex-1">
        {selected ? (
          <FilePreview key={selected.path} entry={selected} onClose={() => setSelected(null)} />
        ) : (
...
function FilePreview({ entry, onClose }: { entry: DirEntry; onClose: () => void }) {
  const ext = extOf(entry.name);
  if (ext === "ipynb") return <NotebookEditor path={entry.path} onClose={onClose} />;
