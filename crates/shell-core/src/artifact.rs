// Read/list/write files the agent produced in the workspace, for artifact
// previews and the file explorer. Strictly sandboxed to the workspace root: a
// path that escapes it is rejected. Host-agnostic: every function takes the
// resolved root; hosts pick it via `ctx::scope_root`.
...
/// base64.
pub fn read_artifact(root: &Path, path: &str) -> Result<ArtifactFile, String> {
    ...
    Ok(ArtifactFile { path: path.to_string(), mime: mime.to_string(), encoding, data, size })
}
...
/// All .ipynb files under `root` (same bounds/skips as the artifact search),
/// newest first.
pub fn list_notebooks(root: &Path) -> Result<Vec<NotebookEntry>, String> {
...
/// List one directory under `root` (non-recursive) for the file explorer.
/// `rel` is a root-relative dir path ("" = the root itself). Hidden entries and
/// heavy build dirs are skipped; directories sort first, then by name.
...
/// Takes a workspace git snapshot on success.
pub fn write_workspace_file(root: &Path, path: &str, content: &str) -> Result<(), String> {
    ...
    crate::git_snapshot::commit_best_effort(root, "Update workspace files");
    Ok(())
}
...
pub fn add_text_to_workspace(ws: &Path, filename: &str, content: &str) -> Result<String, String> {
...
/// Write raw bytes into the workspace under `filename` (deduplicated on
/// collision) — the upload path a browser host uses in place of the native
/// file-picker copy. Returns the actual name written. The caller takes one git
/// snapshot after a batch.
pub fn add_bytes_to_workspace(ws: &Path, filename: &str, bytes: &[u8]) -> Result<String, String> {
    let base = Path::new(filename)
        .file_name()
        .ok_or("invalid file name")?
        .to_string_lossy()
...
/// Minimal std-only base64 (avoids adding a dependency). Also used by the web
/// server to build the sidecar's Basic-auth header.
