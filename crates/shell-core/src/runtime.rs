// Everything needed to run the Codex-backed APEX Runtime in an app-private
...
/// profile's global skills dir (`<xdg-config>/apex-runtime/skills/`). `skills/`
/// is the external ai4s-skills
...
/// workspace's own `.agents/skills/` stays reserved for skills the user
/// installs. Runs before every sidecar start so app upgrades refresh the packs.
fn deploy_bundled_skills(ctx: &ShellCtx) {
    let dst = ctx.xdg_config_home().join("apex-runtime").join("skills");
    ...
    // live in the workspace's `.agents/skills/`), so any skill dir not in the
...
/// The recipe for one APEX Runtime spawn: bridge arguments, environment,
...
/// permissions) — the host only has to launch its bundled Codex bridge.
...
pub fn build_sidecar_spec(ctx: &ShellCtx, port: u16) -> Result<SidecarSpec, String> {
    ...
    // Run the agent inside the user-facing workspace, NOT the app's cwd (which is `/`
    ...
    // Ship the bundled scientific skills into the app-private runtime profile.
    ...
    if let Some(seeded) = crate::runtime_config::seed_default_permission(&existing) {
        if let Some(dir) = cfg_file.parent() {
            std::fs::create_dir_all(dir).map_err(|e| e.to_string())?;
        }
        std::fs::write(&cfg_file, seeded).map_err(|e| e.to_string())?;
    }
    // Secrets live under the runtime root (provider and connector settings) —
    // owner-only on every start, so
    ...
    let mut envs: Vec<(String, String)> = vec![
        ...
        (
            "APEX_RUNTIME_PASSWORD".into(),
            server_password().to_string(),
        ),
        // App-private dirs: the bridge never touches user-level configuration.
...
/// required). `path` must be absolute. No sidecar restart: APEX Runtime serves
...
pub fn get_approval_mode(ctx: &ShellCtx) -> Result<String, String> {
    ...
    Ok(crate::runtime_config::permission_mode_of(&existing)
        .unwrap_or(crate::runtime_config::MODE_APPROVE)
        .to_string())
}
...
pub fn set_approval_mode(ctx: &ShellCtx, mode: &str) -> Result<PathBuf, String> {
    ...
    let updated = crate::runtime_config::set_permission_mode(&existing, mode)?;
