...
pub struct EnvInfo {
    ...
    pub platform: String,
    /// APEX Discovery app version that recorded this.
    pub app: String,
...
fn pip_freeze(python: &str) -> Option<String> {
...
fn python_version(python: &str) -> Option<String> {
    ...
    CACHE
        .get_or_init(|| {
            let out = quiet_command(python).arg("--version").output().ok()?;
            let text = String::from_utf8_lossy(&out.stdout).trim().to_string();
...
/// Capture the runtime environment for a record. `python` is the interpreter
/// agent code runs on, resolved by the host (the desktop shares the kernel's
/// resolution; the server resolves from its own env) — None when unavailable.
pub fn capture_env(python: Option<&str>, root: &Path, app_version: &str) -> EnvInfo {
    // Package capture is best-effort: no pip / write failure just omits it.
    let packages = python
        .and_then(pip_freeze)
        .and_then(|f| write_lockfile(root, &f).ok());
    EnvInfo {
        python: python.and_then(python_version),
...
pub fn read_env_lockfile(root: &Path, hash: &str) -> Result<String, String> {
    ...
    let path = root.join(STORE_DIR).join(ENV_DIR).join(format!("{hash}.txt"));
